...
exports.category_list = asyncHandler(async (req, res, next) => {
	const allCategories = await Category.find({}, "name").sort({ name: 1 }).populate("name").exec();
...
exports.category_detail = asyncHandler(async (req, res, next) => {
	// Get details of category and all associated books (in parallel)
	const [category, itemsInCategory] = await Promise.all([
		Category.findById(req.params.id).exec(),
		Item.find({ category: req.params.id }, "name amount").exec(),
	]);
	if (category === null) {
		// No results.
...
exports.category_create_post = [
	...
	body("name", "Category must have at least one character and no more than 20 characters")
		.trim()
		.isLength({ max: 20 })
		.escape(),
...
exports.category_update_post = [
	...
	body("category", "Category name must not be empty and contain no more than 20 characters")
		.trim()
		.isLength({ max: 20 })
		.escape(),
